...
export class ProductsService {
  ...
  async update(id: string, updateProductDto: Partial<UpdateProductDto>, currentUser: UserEntity): Promise<ProductEntity> {
  ...
  async updateStock(id: string, stock: number, status: string) {
    ...
    if (status === OrderStatus.DELIVERED) {
      if (product.stock < stock) {
        throw new NotFoundException(`There is not enough stock available for the product with ID ${id}`);
      }
      product.stock -= stock
    } else {
      product.stock += stock
